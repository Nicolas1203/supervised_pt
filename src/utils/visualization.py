...
def plot_confusion_matrix(
    ...
    print(cf_matrix)
    class_tots = np.sum(cf_matrix, axis=0)
    df_cm = pd.DataFrame(cf_matrix/class_tots, index=[i for i in classes],
                        columns=[classes[i] + f"({class_tots[i]})" for i in range(len(classes))])
